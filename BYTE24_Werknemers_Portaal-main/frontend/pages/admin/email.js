...
export default function Email() {
  ...
  const sendMailToTrashbin = async () => {
    // Verwijderen van email (doorsturen naar deleted items)
    try {
      await axios.post(
...
const selectMail = async () => {
  // selecteren van een email en aan het rechter venster laten zien
  try {
    await axios.post(
...
*/
  ...
  /*
const sendMailToArchive = async () => {
  // archiveren van een email
  try {
    await axios.post(
      `${API_URL}/admin/archive-email`,
      {
        id: clickedEmail,
      },
      ApiConfig
                              ...
                              <div className="mt-2">
                                <form className="relative">
                                  ...
                                  <div className="absolute bottom-0 inset-x-px">
                                    {/* Selecting certain employee to receive the email */}
