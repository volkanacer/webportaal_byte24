...
export default function App() {
  ...
  const [beginRooster, setBeginRooster] = useState("");
  const [title, setTitle] = useState("");
  const [endRooster, setEndRooster] = useState("");
  ...
  const getRooster = async () => {
    // Event ophalen vanuit backend voor de rooster
    try {
      const response = await axios.get(`${API_URL}/admin/rooster`, ApiConfig);
      const res = await response.data.map((ro) => {
        return {
          id: ro.id,
          title: ro.title,
          start: new Date(ro.start),
          end: new Date(ro.end),
          color: ro.color,
        };
      });
  ...
  const addRooster = async () => {
    // Rooster laten zien op frontend
    try {
      await axios.post(
        `${API_URL}/admin/rooster-toevoegen`,
        {
          title: title,
          start_hour: beginRooster,
          end_hour: endRooster,
          user_id: user.id,
          color: color,
        },
  ...
  const onDelete = async () => {
    // Verwijderen van een event & vragen of de gebruiker het werkelijk wil verwijderen.
    try {
      await axios.post(
        `${API_URL}/admin/rooster-verwijderen`,
        {
          id: clickedEvent.id,
        },
        ApiConfig
            ...
            <section class="py-8 px-6">
              <div class="flex flex-wrap -mx-3 items-center">
                <div class="w-full lg:w-1/2 flex items-center mb-5 lg:mb-0 px-3">
                  ...
                  <div>
                    <h2 class="mb-1 text-2xl font-bold">Agenda</h2>
                  </div>
                </div>
              </div>
            </section>

            <div className="pl-10 pd-10">
              <h2>Add New Event</h2>
              <div>
                <input
                  className="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-2/12 shadow-sm sm:text-sm border-gray-300 rounded-md"
                  type="text"
                ...
                <Listbox value={user} onChange={setUser}>
                  <div className="mt-1 relative">
                    <Listbox.Button className="bg-white  w-2/12  rounded-md shadow-sm py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                      <span className="block truncate mt-1 focus:ring-indigo-500 sm:text-sm focus:border-indigo-500 rounded-md">
                        {user.name}
                      </span>
                    </Listbox.Button>
                    ...
                    <Transition
                      ...
                      leaveTo="opacity-0"
                    >
                      <Listbox.Options className="absolute z-10 mt-1 w-2/12 bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                        {Users.map((person) => (
                          <Listbox.Option
                            key={person.id}
                            className={({ active }) =>
                ...
                <DatePicker
                  ...
                  className="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-2/12 shadow-sm sm:text-sm border-gray-300 rounded-md"
                  selected={beginRooster}
                  onChange={(start_hour) => {
                    setBeginRooster(start_hour);
                    return start_hour;
                  }}
                  locale="en-US"
                  showTimeSelect
